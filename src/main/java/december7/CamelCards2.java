...
public class CamelCards2 {
    ...
    public static void main(String[] args) {
        ...
        final String CONTENT = AdventHelper.readFile("./src/main/java/december7/input.txt");
    ...
    private static int getHandTypeOfCurrentHand(List<Integer> cardList) {
        ...
        Map<Integer, Long> counts = cardList.stream().collect( Collectors.groupingBy( val -> val, Collectors.counting()));
        System.out.println("counts " + counts);

        if( counts.containsKey( Card.JOKER.ordinal() ) && counts.values().size() > 1 ){

            Long jokerCount = counts.get( Card.JOKER.ordinal() );
            boolean isJokerCountHighestCount = counts.get( Card.JOKER.ordinal() ) >= ( counts.values().stream().mapToLong(val -> val).max().getAsLong() );

            if ( !isJokerCountHighestCount ) {

                //figure out the best possible combination
                // create sorted StringBuilder with cardValues
                counts.values().stream().filter(val -> val != 0).sorted().forEach(cardType::append);

                //sum the maximum of current StringBuilder with jokerCount
                long maxOfSortedString =  Long.parseLong( cardType.substring( cardType.length()-1 ) );
                cardType.replace(cardType.length()-1, cardType.length(), maxOfSortedString + jokerCount + "" );

                //remove jokerCount once from the current String
                int jokerCountIndex = cardType.indexOf( jokerCount + "" );
                cardType.delete( jokerCountIndex, jokerCountIndex + 1 );

                //joker is the highest count (last index of StringBuilder)
            } else {
                counts.values().stream().filter(val -> val != 0).sorted().forEach(cardType::append);
                System.out.println(cardType.length());

                cardType.deleteCharAt( cardType.length()-1 );
                System.out.println(cardType.length());

                //TODO: why do these 3 lines break when altering the "replacement" part or construction of it
                String valueToBeReplaced = String.valueOf( cardType.charAt(cardType.length()-1) );
                Long replacement = Long.parseLong(valueToBeReplaced) + jokerCount;
                cardType.replace( cardType.length()-1, cardType.length(), replacement+"" );
            }

        } else { counts.values().stream().filter(val -> val != 0).sorted().forEach(cardType::append); }
